...
class Puzzle(object):
    ...
    def _exclude_duplicates(self):
        ...
        for cat in self._vals.keys():
            ...
            for val in self._vals[cat]:
                val_rule = CNF()
                val_rule.add_clause(*[self._prop(cat, val, i) for i in range(self._n)])
        ...
        for cat in self._vals.keys():
            ...
            for pos in range(self._n):
                pos_rule = CNF()
                pos_rule.add_clause(*[self._prop(cat, val, pos) for val in self._vals[cat]])
    ...
    def coincidence(self, cat_a: str, val_a: str, cat_b: str, val_b: str):
        ...
        for i in range(self._n):
            rule.add_clause(self._prop(cat_a, val_a, i), self._prop(cat_b, val_b, i))
    ...
    def adjacent(self, cat_a: str, val_a: str, cat_b: str, val_b: str):
        ...
        for i in range(self._n - 1):
            rule.add_clause(self._prop(cat_a, val_a, i), self._prop(cat_b, val_b, i + 1))
            rule.add_clause(self._prop(cat_a, val_a, i + 1), self._prop(cat_b, val_b, i))
    ...
    def consecutive(self, cat_1: str, val_1: str, cat_2: str, val_2: str):
        ...
        for i in range(self._n - 1):
            rule.add_clause(self._prop(cat_1, val_1, i), self._prop(cat_2, val_2, i + 1))
